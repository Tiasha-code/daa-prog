...
void prims(Graph *graph, Node *queue, int *idx, int s)
{
    ...
    while ((*idx) != -1) // Empty
    {
        ...
        for (i = 0; i < graph->adjList[u.idx].deg; i++)
        {
            v = graph->adjList[u.idx].arr[i].dest;
            if (isInQueue(queue, graph->numVertices, v) && graph->adjList[u.idx].arr[i].weight < graph->vertices[v].key)
            {
                graph->vertices[v].parent = u.idx;
                graph->vertices[v].key = graph->adjList[u.idx].arr[i].weight;
                for (j = 0; j < (*idx); j++)
                {
                    if (v == queue[j].idx)
                        break;
                }
                heapDecreaseKey(queue, j, graph->adjList[u.idx].arr[i].weight, graph->numVertices, (*idx));
            }
        }
    }
}
...
void printMST(Graph *graph)
{
    ...
    for (v = 0; v < graph->numVertices; v++)
    {
        printf("%d\t\t\t%d\t\t  %d\n", v, graph->vertices[v].parent, graph->vertices[v].key);
    }
    printf("\n");
}
...
int main()
{
    ...
    scanf("%d", &s);
    prims(graph, queue, &i, s);
    printf("\nAfter Prims Algo -> The MST with src. %d:\n", s);
    printMST(graph);
